...
// create obj array 
  const searchArray = [{ name : "Cafe-nora",
              screenshot : ProjectOne,
              tech : "HTML, CSS, Figma, Embedded Google Maps, Font Awesome icons",
              // tech1 : [ "HTML", "CSS", "Figma", "Embedded Google Maps", "Font Awesome icons" ],
          ...
          <>
            <div className="w-50 mx-auto">
              <input
                ...
                value={search}
                onChange = {(e)=> setSearch(e.target.value)}  //input value set to setSearch
              />
            </div>
         {/* map & filter search array  */}
           {
            searchArray.filter((project) => project.name.toLowerCase().includes(search.toLowerCase())).map((item , key) => (
                 <Projects
              name={item.name}
